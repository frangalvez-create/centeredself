...
Input: {content}
Output: Provide only a succinct, information-dense summary capturing the essence of recent behavioral science and therapy modalities relevant to the search terms. The summary must be concise, in 2 short paragraphs. The first paragraph must empathetically acknowledge/summarize the search term concerns, then provide fact-based analysis. The second paragraph must provide achievable actions the users can implement to address the concern and the goal to be {goal}. Limit the bulleted actions to no more than 3. End with related, "quote" from well known figures OR inspirational "affirmation". (Note: If client input and goal is dangerous and abusive, respond positively and provide crisis lifelines.)
Constraints: Focus on capturing the main points succinctly: complete sentences and in a conversational empathetic and analytical tone. Ignore fluff, background information. Do not include your own analysis or opinion. Do not reiterate the input.
